...
HaversineFormula::HaversineFormula(double bearing)
    : CoordinatesConverterBase(bearing) {}

double HaversineFormula::getDistance(double latitude1, double longitude1,
  ...
  double a = sin(dLat / 2) * sin(dLat / 2) + cos(latitude1 * DEG2RAD) *
                                                 cos(latitude2 * DEG2RAD) *
                                                 sin(dLon / 2) * sin(dLon / 2);
  double c = 2 * atan2(sqrt(a), sqrt(1 - a));
  return R * c;
}

double HaversineFormula::getBearing(double latitude1, double longitude1,
                                    double latitude2, double longitude2) {
  /*   Haversine formula:
   *   a = sin²(Δφ/2) + cos φ1 ⋅ cos φ2 ⋅ sin²(Δλ/2)
   *   c = 2 ⋅ atan2( √a, √(1−a) )
