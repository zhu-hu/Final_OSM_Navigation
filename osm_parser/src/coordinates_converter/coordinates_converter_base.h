...
class CoordinatesConverterBase {
 public:
  ...
  virtual double getDistance(double latitude1, double longitude1,
                             double latitude2, double longitude2) = 0;

  virtual double getBearing(double latitude1, double longitude1,
                            double latitude2, double longitude2) = 0;
  ...
  void setOffset(double offset) { this->offset_ = offset; }
  ...
  double getDistance(N1 node1, N2 node2) {
    return getDistance(node1.latitude, node1.longitude, node2.latitude,
                       node2.longitude);
  };

  template <class N1, class N2>
  double getDistance(N1 node) {
    return getDistance(origin_.latitude, origin_.longitude, node.latitude,
                       node.longitude);
  };

  template <class N1, class N2>
  double getBearing(N1 node1, N2 node2) {
    return getBearing(node1.latitude, node1.longitude, node2.latitude,
                      node2.longitude);
  };

  template <class N>
  double getBearing(N node) {
    return getBearing(origin_.latitude, origin_.longitude, node.latitude,
                      node.longitude);
  };
  ...
  double getCoordinateX(double latitude1, double longitude1, double latitude2,
                        double longitude2) {
    GeoNode node1, node2;
  ...
  double getCoordinateY(double latitude1, double longitude1, double latitude2,
                        double longitude2) {
    GeoNode node1, node2;
    node1.latitude = latitude1;
    node1.longitude = longitude1;
