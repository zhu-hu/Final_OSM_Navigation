...
#include "coordinates_converter_base.h"
...
class WGS84Elipsoid : public CoordinatesConverterBase {
 public:
  ...
  double getDistance(double latitude1, double longitude1, double latitude2,
                     double longitude2) override;
  double getBearing(double latitude1, double longitude1, double latitude2,
                    double longitude2) override;

 private:
  geometry_msgs::Point point_;

  constexpr static double a =
      6375137;  // m - dlzka hlavnej polosi, polomer rovnika
  constexpr static double b =
      6356753;  // m - dlzka vedlajsej polosi, polomer poludnika
